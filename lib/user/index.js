    ...
    _checkRequired(data, 
        ...
        options._Service.User.create(userDocument, function (err, result) {
            ...
            options._Service.User.get(
                ...
                var token = res.jsonld.email.id.substr(2);

                // build the activation url
                var activationUrl = 'https://' + options.socket.upgradeReq.headers.host + '/activate/' + token;
 ...
 * @param {Object} data An object containing the following fields:
 *
 * - `token` (String): The email activation token (required).
 *
 * @param {Function} next The next function.
 */
exports.activateEmail = function (options, data, next) {
    ...
    if (!data.token) {
        return next('Service-api.user.activateEmail: No activation token provided.');
    }
    var emailId = '_:' + data.token;
    ...
    options._Service.User.get(
        ...
        if (!res || !res.jsonld || !res.jsonld.email) {
        ...
        // update user email
        options._Service.User.update(['email'], res.jsonld.id, updateData, next);
    });
};
...
exports.get = function (options, data, next) {
    ...
    options._Service.User.get(
        ...
        if (!res.jsonld.image) {
            res.jsonld.image = 'https://thebenclark.files.wordpress.com/2014/03/facebook-default-no-profile-pic.jpg';
        }
 ...
 * @param {Function} next The next function.
 */
exports.changeProfilePic = function (chain, options, onError) {
    options.session = options.session || {};
    ...
    // parse the request
    form.parse(options.req, function (err, fields, files) {

        if (err) {
            chain.o.emit('error', new Error('Service-api.user.changeProfilePic: ' + err.message));
            chain.o.end();
            return;
        }
        ...
        if (!file || !file.size) {
            chain.o.emit('error', new Error('Service-api.user.changeProfilePic: File not found.'));
            chain.o.end();
            return;
        }

        // upload image to jipics
        jipics.upload({
            path: file.path,
            ...
            var imageUrl = 'http://jipics.net/' + data[0];
            ...
            // call service-api
            options._Service.User.update(fields, options.session.id, updateData, function (err) {
 ...
 * @param {Function} next The next function.
 */
exports.removeEmail = function (options, data, next) {
    options.session = options.session || {};

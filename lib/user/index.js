    ...
    _checkRequired(data, 
        ...
        options._Service.User.create(userDocument, function (err, result) {
            ...
            options._Service.User.get(
                ...
                var activationUrl = 'https://' + options.socket.upgradeReq.headers.host + '/flow/service_api:user.activateEmail/' + emailId;
...
exports.activateEmail = function (options, data, next) {
    ...
    options._Service.User.get(
        ...
        if (!res || !res.jsonld) {
            return next(new Error('Service-api.user.activateEmail: emailId not valid.'));
        }

        console.log(res.jsonld);
    });
};
...
exports.sendEmailRecovery = function (options, data, next) {
    ...
    _checkRequired(data,
        ...
        options._Service.User.get( 
            ...
            var recoveryUrl = 'https://' + options.socket.upgradeReq.headers.host + '/flow/service_api:user.resetPassword'
